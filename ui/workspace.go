...
func MetadataPath(base, wf string, idx int) string {
	return filepath.Join(ProcessedDir(base, wf, idx), "metadata.json")
}
func ReasoningPath(base, wf string, idx int) string {
	return filepath.Join(ProcessedDir(base, wf, idx), "metadata_reasoning.md")
}
func FinalVideoPath(base, wf string, idx int) string {
	return filepath.Join(ProcessedDir(base, wf, idx), "final.mp4")
}
func FinalSRTPath(base, wf string, idx int) string {
	return filepath.Join(ProcessedDir(base, wf, idx), "final.srt")
}
func ThumbnailPath(base, wf string, idx int) string {
	return filepath.Join(ProcessedDir(base, wf, idx), "thumbnail.jpg")
}

func EventPath(base, wf string) string { return filepath.Join(WorkspacePath(base, wf), "event.json") }
func BumpersPath(base, wf string) string {
	return filepath.Join(WorkspacePath(base, wf), "bumpers.json")
}
